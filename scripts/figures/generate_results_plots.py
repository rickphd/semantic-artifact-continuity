...
def plot_module_sensitivity() -> None:
    data = pd.read_csv(ABLATION / "module_ablation_summary.csv")
    data = data[data["condition"].str.startswith("drop_")].copy()
    data["module"] = data["condition"].str.removeprefix("drop_")
    pivot = data.pivot(index="module", columns="model", values="delta_vs_full37").reindex(columns=MODELS[:3])
    fig, ax = plt.subplots(figsize=(8.2, 5.2), dpi=300)
    image = ax.imshow(pivot.to_numpy(), cmap="RdBu_r", vmin=-0.08, vmax=0.08)
    ax.set_xticks(range(3), MODELS[:3])
...
def plot_module_coverage() -> None:
    ...
    ax.set_xlim(0, max(100, data["active_pct"].max() * 1.12))
...
def write_provenance_manifest() -> None:
    ...
    source_groups = {
        ...
        "fig10_module_sensitivity": [ABLATION / "module_ablation_summary.csv"],
