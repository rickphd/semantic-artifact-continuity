# Coverage And Sparsity Report

Rows: `1614`
Ontology variables: `37`
Selected train only variables: `6`
Posts with any ontology activation: `1489` (92.26%)
Posts with selected variable activation: `1408` (87.24%)

## Split Activation

- `train`: 93.39%
- `val`: 91.02%
- `test`: 90.09%

## Module Coverage

- `ai`: 954 posts (59.11%), selected features 1
- `ml`: 36 posts (2.23%), selected features 0
- `technology`: 232 posts (14.37%), selected features 1
- `future`: 152 posts (9.42%), selected features 0
- `data`: 231 posts (14.31%), selected features 0
- `algorithm`: 21 posts (1.30%), selected features 0
- `robot`: 125 posts (7.74%), selected features 0
- `automation`: 46 posts (2.85%), selected features 0
- `ethics`: 57 posts (3.53%), selected features 1
- `innovation`: 47 posts (2.91%), selected features 1
- `global_counts`: 1489 posts (92.26%), selected features 2
